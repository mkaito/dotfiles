...
add_binds("normal", {
    ...
    key({}, "v", "Send hovered or current url to clive.",
        function (w) 
            local view = w.view
            local uri = view.hovered_uri or view.uri
            print(string.format("Sending %q to clive", uri))
            if uri then
                luakit.spawn(string.format("urxvtc -T luakit-clive -e clive -f best %q --exec 'mv %%f ~/Downloads'", uri))
            end
        end),
})
