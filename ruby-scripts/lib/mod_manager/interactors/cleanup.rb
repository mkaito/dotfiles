    ...
    class Cleanup
      ...
      def call(dry_run: false, yes: false)
        ...
        @catalog.list_collections.each do |col_name|
          @catalog.read_collection(col_name).mods.each { referenced << _1 }
        rescue ValidationError => e
          e.errors.each { @terminal.warn("warning: #{_1}") }
        end
        @catalog.list_modsets.each do |ms_name|
          @catalog.read_modset(ms_name).mods.each { referenced << _1 }
        rescue ValidationError => e
          e.errors.each { @terminal.warn("warning: #{_1}") }
        end
