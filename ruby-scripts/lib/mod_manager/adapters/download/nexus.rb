    ...
    module Download
      class Nexus
        ...
        def list_files(mod_id:)
          ...
          ::Nexus::FilePicker.sort_files(raw).map { Translator.file_info(it) }
        end
        ...
        def fetch(mod_id:, file_id:, slug: nil)
          ...
          file = files.find { it["file_id"] == file_id } ||
            @client.mod_file(@game_domain, mod_id.to_i, file_id)

          version = file["version"].to_s.strip.then { it.empty? ? info["version"].to_s.strip : it }
        ...
        def pick_url(urls)
          ...
          preferred = urls.find { it["short_name"] == "Nexus CDN" }
