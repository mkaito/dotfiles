...
class ModModsetIntegrationTest < Minitest::Test
  ...
  def test_modset_new_creates_file
    status, out = run_mod("modset", "new", "full")
  ...
  def test_modset_new_errors_if_exists
    run_mod("modset", "new", "full")
    status, out = run_mod("modset", "new", "full")
    assert_equal 1, status
    assert_match(/already exists/, out)
  end

  def test_modset_list_empty
    status, out = run_mod("modset", "list")
    assert_equal 0, status, out
    assert_match(/no modsets/, out)
  end

  def test_modset_list_shows_name_and_count
    run_mod("modset", "new", "full")
    run_mod("modset", "add", "full", "frameworks")
    status, out = run_mod("modset", "list")
    assert_equal 0, status, out
    assert_match(/full.*1 collection/, out)
  end

  def test_modset_show_empty
    run_mod("modset", "new", "full")
    status, out = run_mod("modset", "show", "full")
    assert_equal 0, status, out
    assert_match(/empty/, out)
  end

  def test_modset_show_lists_collections
    run_mod("modset", "new", "full")
    run_mod("modset", "add", "full", "frameworks", "ui-mods")
    status, out = run_mod("modset", "show", "full")
    assert_equal 0, status, out
  ...
  def test_modset_add_appends_collection
    run_mod("modset", "new", "full")
    status, out = run_mod("modset", "add", "full", "frameworks")
    assert_equal 0, status, out
    assert_equal ["frameworks"], TomlRB.load_file(rs_path("full"))["collections"]
  end

  def test_modset_add_multiple_collections
    run_mod("modset", "new", "full")
    status, out = run_mod("modset", "add", "full", "frameworks", "ui-mods")
    assert_equal 0, status, out
    assert_equal ["frameworks", "ui-mods"], TomlRB.load_file(rs_path("full"))["collections"]
  end

  def test_modset_add_idempotent
    run_mod("modset", "new", "full")
    run_mod("modset", "add", "full", "frameworks")
    run_mod("modset", "add", "full", "frameworks")
    assert_equal ["frameworks"], TomlRB.load_file(rs_path("full"))["collections"]
  ...
  def test_modset_add_errors_on_unknown_collection
    run_mod("modset", "new", "full")
    status, out = run_mod("modset", "add", "full", "no-such-collection")
    assert_equal 1, status
    assert_match(/collection not found/, out)
  end

  def test_modset_remove_drops_collection
    run_mod("modset", "new", "full")
    run_mod("modset", "add", "full", "frameworks", "ui-mods")
    status, out = run_mod("modset", "remove", "full", "frameworks")
    assert_equal 0, status, out
    assert_equal ["ui-mods"], TomlRB.load_file(rs_path("full"))["collections"]
  end

  def test_modset_delete_removes_file
    run_mod("modset", "new", "full")
    status, out = run_mod("modset", "delete", "-y", "full")
    assert_equal 0, status, out
    refute File.exist?(rs_path("full"))
  end

  def test_modset_delete_errors_when_not_found
    status, out = run_mod("modset", "delete", "-y", "nonexistent")
    assert_equal 1, status
    assert_match(/not found/, out)
  end

  def test_full_flow
    run_mod("modset", "new", "my-setup")
    run_mod("modset", "add", "my-setup", "frameworks", "ui-mods")

    _, out = run_mod("modset", "show", "my-setup")
    assert_match(/frameworks/, out)
    assert_match(/ui-mods/, out)

    _, out = run_mod("modset", "list")
    assert_match(/my-setup.*2 collection/, out)

    run_mod("modset", "remove", "my-setup", "frameworks")
    _, out = run_mod("modset", "show", "my-setup")
    refute_match(/frameworks/, out)
    assert_match(/ui-mods/, out)

    run_mod("modset", "delete", "-y", "my-setup")
