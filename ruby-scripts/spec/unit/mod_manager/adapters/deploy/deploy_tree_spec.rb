...
class DeployTreeTest < Minitest::Test
  ...
  def test_two_mods_separate_top_dirs_produce_two_dir_symlinks
    ...
    assert result.all? { it[:dir] }
    assert_equal %w[archive r6], result.map { it[:dst_rel] }.sort
  end
  ...
  def test_two_mods_shared_parent_exclusive_children
    ...
    dir_links = result.select { it[:dir] }
    file_links = result.reject { it[:dir] }

    assert_empty file_links
    dst_rels = dir_links.map { it[:dst_rel] }.sort
    assert_equal [
      "bin/x64/plugins/cet/mods/nativeSettings",
      "bin/x64/plugins/cet/mods/vehicleHandling"
    ], dst_rels

    native = dir_links.find { it[:dst_rel].end_with?("nativeSettings") }
    assert_equal "cp2077/native/1.0/bin/x64/plugins/cet/mods/nativeSettings", native[:src_rel]

    vhs = dir_links.find { it[:dst_rel].end_with?("vehicleHandling") }
  ...
  def test_two_exclusive_subdirs_under_shared_parent
    ...
    assert_equal ["r6/scripts/mod-a", "r6/scripts/mod-b"], result.select { it[:dir] }.map { it[:dst_rel] }.sort
    assert_empty result.reject { it[:dir] }
  end
  ...
  def test_loose_file_in_shared_dir_produces_file_symlink
    ...
    assert_equal ["r6/scripts/mod-b"], result.select { it[:dir] }.map { it[:dst_rel] }.sort
    assert_equal ["r6/scripts/loose.reds"], result.reject { it[:dir] }.map { it[:dst_rel] }.sort
  end
  ...
  def test_exclusive_subdir_deep_in_shared_tree
    ...
    assert_equal %w[a/b/c a/b/e], result.select { it[:dir] }.map { it[:dst_rel] }.sort
    assert_empty result.reject { it[:dir] }
  end
end
